...
def check_mrc_dimensions_and_plot(path):
    try:
        with mrcfile.open(path, permissive=True) as mrc:
            ...
            if data.ndim == 2:
                img = data  # already a sinogram or 2D slice

            elif data.ndim == 3:
                # Remove singleton dimensions (e.g., (1, y, x))
                img = np.squeeze(data)

                print(f"Squeezed shape: {img.shape}")

                if img.ndim == 3:
                    # Likely a tilt series: (N_images, Y, X)
                    # Plot the first image or extract a sinogram
