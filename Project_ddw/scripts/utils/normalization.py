...
def get_avg_model_input_mean_and_std(tomo_file, subtomo_size, subtomo_extraction_strides, standardize, mw_angle, batch_size, num_workers, batches=None, verbose=False):
    """
    Computes the average mean and standard deviation of model-input-type patches (with two missing wedges). 
    These values are used to normalize patches during model fitting and to normalize full tomograms in the final refinement step. 
    """
...
def get_avg_model_input_mean_and_std_from_dataloader(dataloader, batches=None, verbose=False):
    ...
    for _ in bar:
        ...
        # Changed dim=(-1, -2, -3) to dim=(-1, -2) for 2D patches
