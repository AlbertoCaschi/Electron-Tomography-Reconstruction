...
from .utils.unet import LitUnet2D  # Changed to 2D
...
def fit_model(
    unet_params_dict: Annotated[
        ...
        typer.Option(
            ...
            help=f"Dictionary of parameters for the U-Net model. See {inspect.getfile(LitUnet2D)} for details.", # Changed to 2D
        ),
    ],
    ...
    subtomo_size: Annotated[
        int, typer.Option(help="Size of the 2D patches used for model fitting.") # Changed description
    ],
    mw_angle: Annotated[
    ...
    if subtomo_dir is None:
        if project_dir is not None:
            subtomo_dir = f"{project_dir}/data/subtomos"
        else:
            raise ValueError(
    ...
    # initialize the model (Changed to LitUnet2D)
    lit_unet = LitUnet2D(
