...
# --------------------------------------------------------
# 1.5 Animated AWS-Style Background
# --------------------------------------------------------
st.markdown("""
    <style>
    /* Push the canvas behind the Streamlit UI */
    .bg-animation-area {
        position: fixed;
        top: 0;
        left: 0;
        width: 100vw;
        height: 100vh;
        z-index: -1; /* Ensures it stays behind your cards and inputs */
        overflow: hidden;
        background: linear-gradient(135deg, #090e17 0%, #111827 100%); /* Deep tech blue/black */
    }
    ...
    .bg-squares li {
        ...
        width: 20px;
        height: 20px;
        background: rgba(255, 255, 255, 0.03); /* Very subtle opacity */
        border: 1px solid rgba(255, 255, 255, 0.05); /* Subtle border like AWS */
        animation: floatUp 25s linear infinite;
        bottom: -150px;
        border-radius: 4px;
    }

    /* Randomizing size, position, duration, and delay for an organic feel */
    .bg-squares li:nth-child(1) { left: 25%; width: 80px; height: 80px; animation-delay: 0s; }
    .bg-squares li:nth-child(2) { left: 10%; width: 30px; height: 30px; animation-delay: 2s; animation-duration: 12s; }
    ...
    @keyframes floatUp {
        ...
        100% {
            ...
            opacity: 0; /* Fade out before hitting the top */
        }
    }
    
    /* Make Streamlit's default background transparent so the animation shows through */
    .stApp {
